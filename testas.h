...
Vektorius<ElementoTipas>::Vektorius(Vektorius&& pagrindinis) noexcept {
    talpa = pagrindinis.talpa;
    dydis = pagrindinis.dydis;
    masyvas = pagrindinis.masyvas;
    
    pagrindinis.masyvas = nullptr; // Nullify the pagrindinis vector's pointer
    pagrindinis.dydis = 0;
    pagrindinis.talpa = 0;
}
...
// template <typename ElementoTipas>
// void Vektorius<ElementoTipas>::print() const {
//     cout << "[";
//     for (size_t i = 0; i < dydis; ++i) {
//         cout << masyvas[i];
//         if (i < dydis - 1) {
...
template <typename ElementoTipas>
bool operator<=(const Vektorius<ElementoTipas>& lhs, const Vektorius<ElementoTipas>& rhs) {
    return !(rhs < lhs); //ar desine nera mazesne uz kaire
}
